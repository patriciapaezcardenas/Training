...
namespace TestNinja.Mocking
{
    public class HousekeeperService
    {
        ...
        {
            var housekeepers = _unitOfWork.Query<Housekeeper>(); // => External Resource (1)
            ...
            foreach (var housekeeper in housekeepers)
            {
                ...
                var statementFilename = _statementGenerator.SaveStatement(housekeeper.Oid, housekeeper.FullName, statementDate);// => External Resource (2)
                ...
                try
                {
                   _emailSender.EmailFile(emailAddress, emailBody, statementFilename,
                        string.Format("Sandpiper Statement {0:yyyy-MM} {1}", statementDate, housekeeper.FullName));// => External Resource (3)
                }
                catch (Exception e)
                {
                    _xtraMessageBox.Show(e.Message, string.Format("Email failure: {0}", emailAddress),
                        MessageBoxButtons.OK);// => External Resource (4)
                }
            }
        }
    }
